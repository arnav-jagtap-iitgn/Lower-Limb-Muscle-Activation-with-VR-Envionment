...
field = vizshape.addPlane(size=(70, 40))  # Field size: 20x20 units
...
# Top Boundary
boundary_top = vizshape.addBox(size=(20, 0.01, 0.1))
boundary_top.setPosition(0, 0.01, 10)     # At the top edge
boundary_top.color(viz.WHITE)

# Bottom Boundary
boundary_bottom = vizshape.addBox(size=(20, 0.01, 0.1))
boundary_bottom.setPosition(0, 0.01, -10) # At the bottom edge
boundary_bottom.color(viz.WHITE)

# Left Boundary
boundary_left = vizshape.addBox(size=(0.1, 0.01, 20))
boundary_left.setPosition(-10, 0.01, 0)  # Left edge
boundary_left.color(viz.WHITE)

# Right Boundary
boundary_right = vizshape.addBox(size=(0.1, 0.01, 20))
boundary_right.setPosition(10, 0.01, 0)  # Right edge
...
goal_left.setPosition(-11, 0.01, 0)  # Position at the lef2 goal
goal_left.setEuler(-90, 0, 0)         # Rotate to face the field
goal_left.setScale(0.004, 0.003, 0.003)

# Place the right goalpost
goal_right.setPosition(11, 0.01, 0)  # Position at the right goal
goal_right.setEuler(90, 0, 0)       # Rotate to face the field
goal_right.setScale(0.004, 0.003, 0.003)

# Place the top goalpost
goal_top.setPosition(0, 0.01, 11)  # Position at the top goal
goal_top.setEuler(0, 0, 0)         # Rotate to face the field
goal_top.setScale(0.004, 0.003, 0.003)
...
field_size = 12 # Half the field size (assumes 20x20 field)
score = 0
...
def checkGoal():
    global score, current_task, field_size, break_in_progress, game_complete, ball_inside_goal, time_inside_goal, last_corner_state
    ...
    if current_task >= len(goal_positions):
        if field_size == field_size_level2:
    ...
    if current_task < len(goal_positions):
        ...
        if current_task_index == 0:
            ...
            goalpost = goal_left if field_size == 12 else goal_left_level2
        elif current_task_index == 1:
            arrow_model.setEuler(0, 90, 0)
            arrow_model.setPosition(0, 0, 2)     # Place it above the field
            goalpost = goal_top if field_size == 12 else goal_top_level2
        else:
            arrow_model.setEuler(0, 90, -90)  
            arrow_model.setPosition(2, 0, 0)     # Place it above the field
            goalpost = goal_right if field_size == 12 else goal_right_level2
        ...
        corner_width = (goal_max[0] - goal_min[0]) * 0.3
        corner_depth = (goal_max[2] - goal_min[2]) * 0.3
        ...
        if current_task_index == 0:  # Left goalpost
            ...
            is_in_bottom_corner = (
                ball_pos[2] <= goal_min[2] + corner_depth and
                goal_min[0] <= ball_pos[0] <= goal_max[0]
            )
            is_in_top_corner = (
                ball_pos[2] >= goal_max[2] - corner_depth and
                goal_min[0] <= ball_pos[0] <= goal_max[0]
            )
        elif current_task_index == 1:  # Top goalpost
            is_in_left_corner = (
                ball_pos[0] <= goal_min[0] + corner_width and
                goal_min[2] <= ball_pos[2] <= goal_max[2]
            )
            is_in_right_corner = (
                ball_pos[0] >= goal_max[0] - corner_width and
                goal_min[2] <= ball_pos[2] <= goal_max[2]
            )
        ...
        if abs(ball_pos[0]) > field_size-1.5 or abs(ball_pos[2]) > field_size-1.5:
            task_score = 40
            score += task_score 
...
def updateBallPosition():
    ...
    ball_x = mouse_x * field_size
    ball_z = mouse_z * field_size
...
field_size_level2 = 17  # Half the field size for Level 2
goal_positions_level2 = [
    (-16, 0.2, 0),  # Left goal for Level 2
    (0, 0.2, 16),   # Top goal for Level 2
    (16, 0.2, 0)    # Right goal for Level 2
]
...
def setupLevel2():
    ...
    # Position and scale Level 2 goalposts
    goal_left_level2.setPosition(-16, 0.01, 0)
    goal_left_level2.setScale(0.004, 0.003, 0.003)
    goal_left_level2.setEuler(-90, 0, 0)

    goal_right_level2.setPosition(16, 0.01, 0)
    goal_right_level2.setScale(0.004, 0.003, 0.003)
    goal_right_level2.setEuler(90, 0, 0)

    goal_top_level2.setPosition(0, 0.01, 16)
    goal_top_level2.setScale(0.004, 0.003, 0.003)
    goal_top_level2.setEuler(0, 0, 0)

    # Create larger boundaries for Level 2
    boundary_objects_level2 = [
        vizshape.addBox(size=(30, 0.01, 0.1)),  # Top boundary
        vizshape.addBox(size=(30, 0.01, 0.1)),  # Bottom boundary
        vizshape.addBox(size=(0.1, 0.01, 30)),  # Left boundary
        vizshape.addBox(size=(0.1, 0.01, 30))   # Right boundary
    ]
    boundary_objects_level2[0].setPosition(0, 0.01, 15)  # Top boundary
    boundary_objects_level2[1].setPosition(0, 0.01, -15)  # Bottom boundary
    boundary_objects_level2[2].setPosition(-15, 0.01, 0)  # Left boundary
    boundary_objects_level2[3].setPosition(15, 0.01, 0)   # Right boundary
    for boundary in boundary_objects_level2:
        boundary.color(viz.WHITE)

def switchToLevel2():
    global field_size, goal_positions, current_task
